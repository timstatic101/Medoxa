...
const EditInsurances = () => {
  ...
  const { insurancesId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: insurancesId }));
  }, [insurancesId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: insurancesId, data }));
...
EditInsurances.getLayout = function getLayout(page: ReactElement) {
  return (
