...
const EditHolidays = () => {
  ...
  const { holidaysId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: holidaysId }));
  }, [holidaysId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: holidaysId, data }));
...
EditHolidays.getLayout = function getLayout(page: ReactElement) {
  return (
