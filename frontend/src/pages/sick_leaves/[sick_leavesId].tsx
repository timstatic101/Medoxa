...
const EditSick_leaves = () => {
  ...
  const { sick_leavesId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: sick_leavesId }));
  }, [sick_leavesId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: sick_leavesId, data }));
...
EditSick_leaves.getLayout = function getLayout(page: ReactElement) {
  return (
...
export default EditSick_leaves;
