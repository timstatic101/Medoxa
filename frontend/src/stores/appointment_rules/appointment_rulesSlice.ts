...
export const appointment_rulesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetch.fulfilled, (state, action) => {
      if (action.payload.rows && action.payload.count >= 0) {
        state.appointment_rules = action.payload.rows;
        state.count = action.payload.count;
      } else {
        state.appointment_rules = action.payload;
      }
      state.loading = false;
    });
...
export const { setRefetch } = appointment_rulesSlice.actions;

export default appointment_rulesSlice.reducer;
