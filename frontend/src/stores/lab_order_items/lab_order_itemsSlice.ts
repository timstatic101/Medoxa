...
export const lab_order_itemsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetch.fulfilled, (state, action) => {
      if (action.payload.rows && action.payload.count >= 0) {
        state.lab_order_items = action.payload.rows;
        state.count = action.payload.count;
      } else {
        state.lab_order_items = action.payload;
      }
      state.loading = false;
    });
    ...
    builder.addCase(deleteItemsByIds.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, 'Lab_order_items has been deleted');
    });
    ...
    builder.addCase(deleteItem.fulfilled, (state) => {
      ...
      fulfilledNotify(
        state,
        `${'Lab_order_items'.slice(0, -1)} has been deleted`,
      );
    });
    ...
    builder.addCase(create.fulfilled, (state) => {
      ...
      fulfilledNotify(
        state,
        `${'Lab_order_items'.slice(0, -1)} has been created`,
      );
    });
    ...
    builder.addCase(update.fulfilled, (state) => {
      ...
      fulfilledNotify(
        state,
        `${'Lab_order_items'.slice(0, -1)} has been updated`,
      );
    });
    ...
    builder.addCase(uploadCsv.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, 'Lab_order_items has been uploaded');
    });
...
export const { setRefetch } = lab_order_itemsSlice.actions;

export default lab_order_itemsSlice.reducer;
