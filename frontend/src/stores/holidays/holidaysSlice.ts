  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetch.fulfilled, (state, action) => {
      if (action.payload.rows && action.payload.count >= 0) {
        state.holidays = action.payload.rows;
    ...
    builder.addCase(deleteItemsByIds.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, 'Holidays has been deleted');
    });
    ...
    builder.addCase(deleteItem.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, `${'Holidays'.slice(0, -1)} has been deleted`);
    });
    ...
    builder.addCase(create.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, `${'Holidays'.slice(0, -1)} has been created`);
    });
    ...
    builder.addCase(update.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, `${'Holidays'.slice(0, -1)} has been updated`);
    });
    ...
    builder.addCase(uploadCsv.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, 'Holidays has been uploaded');
    });
...
export const { setRefetch } = holidaysSlice.actions;
